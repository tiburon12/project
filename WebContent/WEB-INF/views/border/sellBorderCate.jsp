...
<%
	...
	request.getSession().setAttribute("returnURL",
			"sellBorderCate.jsp?category=" + category + "&pages=" + pages);
%>
...
<body>
		...
		<div id="body" class="width"
			...
			<table cellspacing="0">
				...
				<tr>
					...
					<td><a
						href="detail.jsp?border=sell&pages=<%=pages%>&seq=<%=list.get(i).getSeq()%>"
			...
			<div style="margin-top: 20px">
				<p style="float: left; margin-left: 40%; width: 45px">
					...
					%>
					<a
						href="sellBorderCate.jsp?category=<%=category%>&pages=<%=startPageNum - 1%>"><img
 ...
 %> <a
					style="font-size: 20px; font-weight: bold;<%if ((startPageNum + i) == pages) {%>
						...
						<%}%>"
					href="sellBorderCate.jsp?category=<%=category%>&pages=<%=startPageNum + i%>"><%=startPageNum + i%></a>
					<%
						}
						}
					%>
				...
				<p style="float: left; width: 45px">
					...
					%>
					<a
						href="sellBorderCate.jsp?category=<%=category%>&pages=<%=startPageNum + 5%>"><img
						style="margin-left: 10px; margin-top: 5px" src="images/next2.jpg" /></a>
					<%
						}
					%>
				</p>
				<span> <a href="write.jsp?border=sell&pages=<%=pages%>"
