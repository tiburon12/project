...
<%
	...
	if (request.getSession().getAttribute("mid") == null) {
		...
		request.getSession().setAttribute("returnURL", "write.jsp?border=" + border + "&pages=" + pages);
		...
		<div id="body" class="width"
			...
			<fieldset>
				...
				<form action="writeProc.jsp" method="post"
					...
					<p>
						<label for="title">제목:</label> <input name="title" id="title" required
					...
					<p>
						<label for="title">파일첨부:</label><input type="file" id="txtFile"
							name="file" />
					</p>

					<p>
						<label for="content">글내용:</label>
						<textarea required rows="20" name="content" id="content" style="width: 80%"></textarea>
					</p>
					<p>
						<input type="hidden" name="border" value="<%=border%>" /> <input
							...
							href="<%-- <%=border%>Border.jsp?pages=<%=pages%> --%><%=returnURL%>"
							class="button"
