...
<body>
	<div id="container">
		...
		<div id="body" class="width"
			...
			<h1 style="margin: 20px; font-size: 50px">글수정</h1>
			<fieldset>
				<legend style="font-size: 30px"><%=border.toUpperCase()%></legend>
				<form action="editNoticeProc.jsp" method="post"
					style="font-size: 20px">
					<p>
						<label for="title">제목:</label> <input name="title" id="title"
							value="<%=b.getTitle()%>" type="text" style="width: 80%" />
					</p>
					<p>
						<label for="category">카테고리:</label> <select name="category">

							<option value="의류"<%if (b.getCategory().equals("의류")) {%>
								selected <%}%>>의류</option>
							<option value="도서"<%if (b.getCategory().equals("도서")) {%>
								selected <%}%>>도서</option>
							<option value="전자제품"<%if (b.getCategory().equals("전자제품")) {%>
								selected <%}%>>전자제품</option>
							<option value="기타" <%if (b.getCategory().equals("기타")) {%>
								selected <%}%>>기타</option>
						</select>
					</p>
					...
					<p>
						...
						<textarea rows="20" name="content" id="content" style="width: 80%"><%=b.getContent()%></textarea>
					</p>
					<p>
						<input type="hidden" name="border" value="<%=border%>" /> <input
							type="hidden" name="pages" value="<%=pages%>" /> <input
							type="hidden" name="seq" value="<%=b.getSeq()%>" /> <input
							name="send" style="margin-left: 75%; margin-top: 20px"
							class="formbutton" value="수정하기" type="submit" /> <a
